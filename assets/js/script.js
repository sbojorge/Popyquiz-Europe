...
/** Displays a question and its multiple choice answer right after the user clicks on
 * the Let's go!, Next or Take the quiz again buttons respectively
 */

 let quiz = myQuiz[Math.floor(Math.random() * myQuiz.length)];
 document.getElementById('question').innerHTML = quiz.q;
 document.getElementById('opt1').innerHTML = quiz.a[0];
 document.getElementById('opt2').innerHTML = quiz.a[1];
 document.getElementById('opt3').innerHTML = quiz.a[2];
...
function handleOptClicked(event) {
  let rightAnswers = quiz.correctAnswer;
